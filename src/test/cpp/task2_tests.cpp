...
namespace csc232
{
    ...
    TEST_F( Task2TestFixture, TaskIsReadyForEvaluation )
    {
        ...
        const char* search_string = "TEST_TASK2 TRUE";
        ...
        if (HasFailure())
        {
            ...
            cerr << "It appears that you have not toggled TEST_TASK2 to TRUE.\n";
        }
    }
#if TEST_TASK2
    TEST_F(Task2TestFixture, ValidateOneDimensionalArrayParameterFunctionDeclared)
    {
        const char* filename   = "csc232.h";
        std::ifstream input_stream{ filename, std::ios::in };
        if (!input_stream)
    ...
    TEST_F( Task2TestFixture, ValidateTwoDimensionalArrayParameterFunctionDeclared)
    {
        const char* filename   = "csc232.h";
        std::ifstream input_stream{ filename, std::ios::in };
        if (!input_stream)
